    ...
    def execute_search(
        ...
        try:
            with get_db_connection() as (conn, cursor):
                ...
                # Get FAISS index and search - KEEPING ORIGINAL SEQUENCE
                ...
                # CRITICAL: Check embedding_ids first as in original
                if not embedding_ids:
                    logger.warning("No embedding IDs provided for query")
                    return []

                # Get and process embedding IDs exactly as in original
                cursor.execute(
                ...
                # Process with FAISS results in exact same way as original
                embedding_ids = [embedding_ids[i] for i in I[0]]

                # Process chunks following original sequence
                query = """SELECT id, small_chunk_id 
                          FROM embeddings 
                          WHERE id IN ({})""".format(
                ...
                # Map IDs exactly as in original
                id_to_small_chunk_id = {id: sc_id for id, sc_id in fetched_data}
                small_chunk_ids = [id_to_small_chunk_id[id]
                                   for id in embedding_ids
                                   if id in id_to_small_chunk_id]
                ...
                # Get big chunks and build results
